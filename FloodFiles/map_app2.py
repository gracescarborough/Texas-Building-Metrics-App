...
GRID_PATH = "/home/grace/FloodFiles/tx_grid_classified_embodied.shp"
CENTROIDS_PATH =
"/home/grace/FloodFiles/sample_centroids_with_stats_and_class_embodied.shp"
@st.cache_data
...
options=["Building Density", "Embodied Energy Intensity (EEI)", "Embodied
Carbon Intensity (ECI)"],
index=0
)
...
hex_data['normalized'] = (hex_data['avg_value'] - vmin) / (vmax - vmin) if vmax >
vmin else 0
...
hex_data['display_value'] = hex_data['avg_value'].round(2).astype(str) + '
MJ/m²'
else:
hex_data['display_value'] = hex_data['avg_value'].round(2).astype(str) + '
kgCO2e/m²'
...
st.subheader("Debug: Data load check")

st.write("Grid path exists:", os.path.exists("tx_grid_classified_embodied.shp"))

grid = gpd.read_file("tx_grid_classified_embodied.shp")

st.write("Grid shape:", grid.shape)
st.write("Columns:", list(grid.columns))
st.write(grid.head())
st.write("Original CRS:", grid.crs)

grid = grid.to_crs(epsg=4326)

st.write("Reprojected CRS:", grid.crs)
