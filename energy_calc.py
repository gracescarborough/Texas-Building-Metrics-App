...
texas_path = "/home/grace/tl_2025_us_state/tl_2025_us_state.shp"
texas_gdf = gpd.read_file(texas_path)
texas_gdf = texas_gdf[texas_gdf["NAME"] == "Texas"]
texas_gdf = texas_gdf.to_crs("EPSG:3083")

GRID_RES_M = 750
half = GRID_RES_M / 2
def centroids_to_polygons(gdf):
    gdf = gdf.copy()
    gdf["geometry"] = [
...
baseline_EE_MJ = 6.58e+12
baseline_EC_kg = 3.9e+11

print("Columns in centroids_gdf:", centroids_gdf.columns.tolist())
res_df = centroids_gdf[centroids_gdf["res_flr_ar"] > 0].copy()
...
# ================================
# CURRENT RESIDENTIAL EE & EC
# ================================

res_current = res_df.copy()

res_current["EE_MJ"] = res_current["flr_area"] * res_current["avg_floors"].apply(
    lambda f: get_eei("Residential", int(max(f, 1)))
)

res_current["EC_kg"] = res_current["flr_area"] * res_current["avg_floors"].apply(
    lambda f: get_eci("Residential", int(max(f, 1)))
)

current_res_EE = res_current["EE_MJ"].sum()
current_res_EC = res_current["EC_kg"].sum()

print("\n=== CURRENT RESIDENTIAL ===")
print(f"EE: {current_res_EE:.2e} MJ")
print(f"EC: {current_res_EC:.2e} kg")
...
def calculate_scenarios(df, new_pop, year):
    ...
    weights2 = candidates2["flr_dens"] ** 3
    if weights2.sum() == 0:
    ...
    candidates3 = df.copy()
    weights3 = candidates3["flr_area"].clip(lower=1e-6)
...
# ================================
# CURRENT DENSITY MAP (for comparison)
# ================================
print("\nGenerating CURRENT density map")
...
# Convert to polygons
current_map_gdf = centroids_to_polygons(res_df)
current_map_gdf["flr_dens"] = current_map_gdf["flr_dens"].fillna(0.0)

current_map_gdf["dens_class"] = pd.cut(
    current_map_gdf["flr_dens"],
    bins = [0, 0.0005, 0.005, 0.01, 0.04, 0.09, 0.48, np.inf],
    labels = ["Very Low", "Low", "Moderate", "Medium", "High", "Very High", "Extreme"],
    include_lowest=True
)
...
texas_gdf.plot(
    ax=ax,
    facecolor="#f7f7e7",  # light grey fill
    edgecolor="none",
    zorder=0
)

for cls in labels:
    subset = current_map_gdf[current_map_gdf["dens_class"] == cls]
    if len(subset) > 0:
...
ax.set_title("Current Residential Building Density")
ax.set_axis_off()

plt.tight_layout()
plt.savefig("texas_density_map_current.png", dpi=300)
plt.show()

print("✅ Current density map saved")

# ============================================================================
# FUTURE DENSITY PLOT
year = 2050
pop_label = "average migration"
scenario_name = "dense"
...
print(f"\nGenerating 2050 map: {scenario_name}, {pop_label}")

res_2050 = calculate_scenarios(res_df, population_by_year[year][pop_label], year)
scenario_df = res_2050[scenario_name][2].copy()

map_gdf = centroids_to_polygons(res_df)

map_gdf["added_floor_area_m2"] = 0.0
map_gdf.loc[scenario_df.index, "added_floor_area_m2"] = scenario_df["added_floor_area_m2"]

map_gdf["future_flr_dens"] = (
    map_gdf["flr_area"] + map_gdf["added_floor_area_m2"]
) / M2_PER_MI2

map_gdf["dens_class"] = pd.cut(
    map_gdf["future_flr_dens"],
    bins=bins,
    labels=labels,
    include_lowest=True
...
for cls in labels:
    subset = map_gdf[map_gdf["dens_class"] == cls]
    if len(subset) > 0:
        subset.plot(ax=ax, color=colors[cls], linewidth=0)
...
ax.set_title(f"2050 Building Density ({scenario_name}, {pop_label})")
ax.set_axis_off()

plt.tight_layout()
plt.savefig(f"texas_density_map_2050_{scenario_name}.png", dpi=300)
plt.show()

print("✅ 2050 map saved")

# ================================
# 2050 CURRENT DISTRIBUTION MAP
# ================================
scenario_name = "current_dist"
...
map_gdf["added_floor_area_m2"] = 0.0
map_gdf["added_floor_area_m2"] = scenario_df["added_floor_area_m2"]
...
print("✅ 2050 current_dist map saved")

# ======================================================================
# 2050 ADDED DENSITY MAPS (ALL SCENARIOS)
# ======================================================================

print("\nGenerating 2050 ADDED density maps (all scenarios)")

year = 2050
pop_label = "average migration"
scenarios = ["sprawl", "dense", "current_dist"]

# Bins for ADDED density (you may want slightly different bins than total density)
added_bins = [0, 0.0001, 0.001, 0.005, 0.01, 0.05, 0.2, np.inf]
added_labels = ["Very Low", "Low", "Moderate", "Medium", "High", "Very High", "Extreme"]
...
for scenario_name in scenarios:
    ...
    # ADDED density only
    map_gdf["added_flr_dens"] = map_gdf["added_floor_area_m2"] / M2_PER_MI2

    # Classify
    map_gdf["dens_class"] = pd.cut(
        map_gdf["added_flr_dens"],
        bins=added_bins,
        labels=added_labels,
        include_lowest=True
    )
    ...
    ax.set_title(f"2050 Added Building Density ({scenario_name}, {pop_label})")
    ax.set_axis_off()

    plt.tight_layout()
    plt.savefig(f"texas_added_density_2050_{scenario_name}.png", dpi=300)
    plt.show()

print("✅ All added density maps saved")
#========================================================================

# ================================
# 2050 RESIDENTIAL EE & EC
# ================================

year = 2050
pop_label = "average migration"

res_2050_results = calculate_scenarios(res_df, population_by_year[year][pop_label], year)

print(f"\n=== 2050 RESIDENTIAL ({pop_label}) ===")

for scenario_name in ["sprawl", "dense", "current_dist"]:

    scenario_df = res_2050_results[scenario_name][2].copy()

    # Merge base + added floor area
    temp = res_df.copy()
    temp["added_floor_area_m2"] = 0.0
    temp.loc[scenario_df.index, "added_floor_area_m2"] = scenario_df["added_floor_area_m2"]

    temp["total_floor_area"] = temp["flr_area"] + temp["added_floor_area_m2"]

    # EE / EC calculation
    temp["EE_MJ"] = temp["total_floor_area"] * temp["avg_floors"].apply(
        lambda f: get_eei("Residential", int(max(f, 1)))
    )

    temp["EC_kg"] = temp["total_floor_area"] * temp["avg_floors"].apply(
        lambda f: get_eci("Residential", int(max(f, 1)))
    )

    total_EE = temp["EE_MJ"].sum()
    total_EC = temp["EC_kg"].sum()

    print(f"\n{scenario_name.upper()}:")
    print(f"EE: {total_EE:.2e} MJ")
    print(f"EC: {total_EC:.2e} kg")
